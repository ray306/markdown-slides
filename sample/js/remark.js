require = function(e, t, n) {
    ...
    return i
            ...
            var EventEmitter = require("events").EventEmitter, styler = require("components/styler");
            var LANDSCAPE = "landscape", PORTRAIT = "portrait", PAGE_HEIGHT = 681, PAGE_WIDTH = 908;
            ...
            function formatSlideNumber(slide, slideshow) {
                var format = slideshow.getSlideNumberFormat(), slides = slideshow.getSlides(), current = getSlideNo(slide, slideshow), total = getSlideNo(slides[slides.length - 1], slideshow);
            ...
            var Api = require("./remark/api"), polyfills = require("./polyfills"), styler = require("components/styler");
            ...
            var resources = require("../../resources"), highlighter = require("../../highlighter");
            module.exports = {styleDocument: styleDocument,setPageSize: setPageSize};
            ...
            function setPageSize(size) {
                var stylesheet = getRemarkStylesheet(), pageRule = getPageRule(stylesheet);
            ...
            var EventEmitter = require("events").EventEmitter, highlighter = require("./highlighter"), converter = require("./converter"), Parser = require("./parser"), Slideshow = require("./models/slideshow"), SlideshowView = require("./views/slideshowView"), DefaultController = require("./controllers/defaultController"), Dom = require("./dom"), macros = require("./macros");
            module.exports = Api;
            function Api(dom) {
            ...
            Api.prototype.convert = function(markdown) {
                var parser = new Parser, content = parser.parse(markdown || "", macros)[0].content;
            ...
            !function() {
                ...
                Parser.prototype.parse = function(src, macros) {
                    var self = this, lexer = new Lexer, tokens = lexer.lex(cleanInput(src)), slides = [], stack = [createSlide()];
                    macros = macros || {};
                    tokens.forEach(function(token) {
                        switch (token.type) {
                            ...
                            case "fences":
                                ...
                                break;
                            case "def":
                                stack[0].links[token.id] = {href: token.href,title: token.title};
                                break;
                            case "macro":
                                var macro = macros[token.name];
                                if (typeof macro !== "function") {
                                    throw new Error('Macro "' + token.name + '" not found. ' + "You need to define macro using remark.macros['" + token.name + "'] = function () { ... };")
                                }
                                ...
                                if (typeof value === "string") {
                                    ...
                                    appendTo(stack[stack.length - 1], value === undefined ? "" : value.toString())
                                }
                                break;
                            case "content_start":
            ...
            var SlideView = require("./slideView"), Timer = require("components/timer"), NotesView = require("./notesView"), Scaler = require("../scaler"), resources = require("../resources"), utils = require("../utils"), printing = require("components/printing");
            ...
            SlideshowView.prototype.configureContainerElement = function(element) {
                ...
                if (element === self.dom.getBodyElement()) {
                    ...
                    forwardEvents(self.events, window, ["hashchange", "resize", "keydown", "keypress", "mousewheel", "message", "DOMMouseScroll"]);
            ...
            SlideshowView.prototype.configureChildElements = function() {
                ...
                function onPrint(e) {
                    var slideHeight;
                    if (e.isPortrait) {
                        slideHeight = e.pageHeight * .4
                    } else {
                        slideHeight = e.pageHeight
                    }
                    self.slideViews.forEach(function(slideView) {
                        slideView.scale({clientWidth: e.pageWidth,clientHeight: slideHeight});
                        if (e.isPortrait) {
            ...
            var Navigation = require("./slideshow/navigation"), Events = require("./slideshow/events"), utils = require("../utils"), Slide = require("./slide"), Parser = require("../parser"), macros = require("../macros");
            module.exports = Slideshow;
            function Slideshow(events, options) {
                var self = this, slides = [], links = {};
                options = options || {};
            ...
            function createSlides(slideshowSource, options) {
                var parser = new Parser, parsedSlides = parser.parse(slideshowSource, macros), slides = [], byName = {}, layoutSlide;
                slides.byName = {};
                parsedSlides.forEach(function(slide, i) {
                    ...
                    if (slide.properties.continued === "true" && options.countIncrementalSlides === false && slide.properties.count === undefined) {
            ...
            !function() {
                ...
                var keyboard = require("./inputs/keyboard"), mouse = require("./inputs/mouse"), touch = require("./inputs/touch"), message = require("./inputs/message"), location = require("./inputs/location");
            ...
            var CODE = 1, INLINE_CODE = 2, CONTENT = 3, FENCES = 4, DEF = 5, DEF_HREF = 6, DEF_TITLE = 7, MACRO = 8, MACRO_ARGS = 9, MACRO_OBJ = 10, SEPARATOR = 11, NOTES_SEPARATOR = 12;
            var regexByName = {CODE: /(?:^|\n)( {4}[^\n]+\n*)+/,INLINE_CODE: /`([^`]+?)`/,CONTENT: /(?:\\)?((?:\.[a-zA-Z_\-][a-zA-Z\-_0-9]*)+)\[/,FENCES: /(?:^|\n) *(`{3,}|~{3,}) *(?:\S+)? *\n(?:[\s\S]+?)\s*\4 *(?:\n+|$)/,DEF: /(?:^|\n) *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +["(]([^\n]+)[")])? *(?:\n+|$)/,MACRO: /!\[:([^\] ]+)([^\]]*)\](?:\(([^\)]*)\))?/,SEPARATOR: /(?:^|\n)(---?)(?:\n|$)/,NOTES_SEPARATOR: /(?:^|\n)(\?{3})(?:\n|$)/};
            var block = replace(/CODE|INLINE_CODE|CONTENT|FENCES|DEF|MACRO|SEPARATOR|NOTES_SEPARATOR/, regexByName), inline = replace(/CODE|INLINE_CODE|CONTENT|FENCES|DEF|MACRO/, regexByName);
            function Lexer() {
            }
            ...
            function lex(src, regex, tokens) {
                ...
                while ((cap = regex.exec(src)) !== null) {
                    ...
                    if (cap[CODE]) {
                        ...
                        tokens.push({type: "def",id: cap[DEF],href: cap[DEF_HREF],title: cap[DEF_TITLE]})
                    } else if (cap[MACRO]) {
                        tokens.push({type: "macro",name: cap[MACRO],args: (cap[MACRO_ARGS] || "").split(",").map(trim),obj: cap[MACRO_OBJ]})
            ...
            function Navigation(events) {
                var self = this, currentSlideIndex = -1, started = null;
                ...
                function gotoSlideByIndex(slideIndex, noMessage) {
                    var alreadyOnSlide = slideIndex === currentSlideIndex, slideOutOfRange = slideIndex < 0 || slideIndex > self.getSlideCount() - 1;
            ...
            NotesView.prototype.showSlide = function(slideIndex) {
                var self = this, slideViews = self.slideViewsAccessor(), slideView = slideViews[slideIndex], nextSlideView = slideViews[slideIndex + 1];
            ...
            var SlideNumber = require("components/slide-number"), converter = require("../converter"), highlighter = require("../highlighter"), utils = require("../utils");
            ...
            SlideView.prototype.scaleBackgroundImage = function(dimensions) {
                var self = this, styles = window.getComputedStyle(this.contentElement), backgroundImage = styles.backgroundImage, match, image, scale;
                    ...
                    image.onload = function() {
                        if (image.width > dimensions.width || image.height > dimensions.height) {
                            if (!self.originalBackgroundSize) {
                                ...
                                self.contentElement.style.backgroundSize = image.width * scale + "px " + image.height * scale + "px";
                                self.contentElement.style.backgroundPosition = "50% " + (dimensions.height - image.height * scale) / 2 + "px"
                            }
                        } else {
            ...
            function highlightBlockSpans(block) {
                var pattern = /([^\\`])`([^`]+?)`/g, replacement = '$1<span class="remark-code-span-highlighted">$2</span>';
            ...
            !function(global) {
                !function() {
                    ...
                    marked.defaults = {gfm: true,tables: true,breaks: false,pedantic: false,sanitize: false,smartLists: false,silent: false,highlight: null,langPrefix: "lang-",smartypants: false,headerPrefix: "",renderer: new Renderer};
